...
s3 >> play('b', sample=[3,4,3,3,4,4,5,6], dur=[1/2], rate=P[1,1,1,1,1,rest(1),rest(2),rest(2)], drive=0.2, amplify=1, amp=0.2)
...
Group(p1,p2,b1).solo(0)


# samples custom
~s2 >> play('b', sample=[3,4,3,3,4,4,5,6], dur=[1/2], rate=P[1,1,2,2,1,1,2,2]*2, drive=0.2, amplify=var([0,1], dur=1/2), amp=0.4)

s3.stop()

s3 >> play('b', sample=[3,4,3,3,4,4,5,6], dur=[1/2], rate=P[1,1,1,1,1,rest(1),rest(2),rest(2)], drive=0.2, amplify=1, amp=0.2)

~s2 >> play('b', sample=[3,4,3,3,4,4,5,6], dur=[1/2], rate=P[1,1,2,2,1,1,2,2]*2, drive=0.2, amplify=var([0,1], dur=1/2), amp=0.3)

s3 >> play('b', sample=[3,4,3,3,4,4,5,6], dur=[1/2], rate=P[1,1,1,1,1,1,1,1,1], drive=0.2, amplify=1, amp=0.2)

p2.stop()

Group(p1,b1,p2,d5).solo()
...
p1 >> pianovel(var.r + P(0,2,4+7,6,9) -7, dur=8, amp=0.8, pan=-0.5)

b1 >> bass(var.r - 7, dur=[3/4,1/4,rest(1/2),1/2,rest(1/2),1/2,rest(1)], sus=[1/4,1/4,1/4,1,1/4,1/4,1/4], amp=0.5)

~p2 >> pianovel(var.r + P[0,2,4,6,7,8], dur=PDur(var([5,3,7], dur=[3,2,3]),8)*2, amp=0.5, pan=0.5).shuffle().stop()

Group(p1,b1,p2).solo(0)
